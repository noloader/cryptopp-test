...
{
	u8 key[16], iv[8], plain[1024], cipher[1024];
	...
	for (unsigned int i=1; i<128; ++i)
	{
		...
		ECRYPT_keysetup(&ctx, key, sizeof(key), sizeof(iv));

		GenRandom(iv, sizeof(iv));
		ECRYPT_ivsetup(&ctx, iv, sizeof(iv));
		
		GenRandom(plain, i);
		const unsigned int blocks = (i+15)/16;
		ECRYPT_process_blocks(1, &ctx, plain, cipher, blocks);
		
		printf("#\n");
				
		printf("Source: rabbit.c reference implementation\n");
		printf("Comment: Rabbit, 128-bit key, 64-bit IV\n");
