/* 
   Unoptimized test implementation of SKIPJACK algorithm 
   Panu Rissanen <bande@lut.fi> 960624

   SKIPJACK and KEA Algorithm Specifications 
   Version 2.0 
   29 May 1998

   ftp://ftp.funet.fi/pub/crypt/cryptography/symmetric/skipjack/skipjack-orig.c
*/

const unsigned char ftable[256] = { 
  
};

unsigned int g(
	const unsigned char *key,
	unsigned int k,
	unsigned int w)
{
  unsigned char g1,g2,g3,g4,g5,g6;

  g1=(w>>8)&0xff;
  g2=w&0xff;

  g3=ftable[g2^key[(4*k)%10]]^g1;
  g4=ftable[g3^key[(4*k+1)%10]]^g2;
  g5=ftable[g4^key[(4*k+2)%10]]^g3;
  g6=ftable[g5^key[(4*k+3)%10]]^g4;

  return ((g5<<8)+g6);
}

#include <stdio.h>
void skipjack_encrypt(
    const unsigned char key[10],
    const unsigned char in[8],
    unsigned char out[8])
{
  unsigned int w1,w2,w3,w4,tmp;
  unsigned int k=0;

#if 1
  w1=(in[0]<<8)+in[1];
  w2=(in[2]<<8)+in[3];
  w3=(in[4]<<8)+in[5];
  w4=(in[6]<<8)+in[7];
#endif

#if 0
  w4=(in[0]<<8)+in[1];
  w3=(in[2]<<8)+in[3];
  w2=(in[4]<<8)+in[5];
  w1=(in[6]<<8)+in[7];
#endif

  //printf("    w1  w2   w3  w4\n   -----------------\n");

  for(size_t t=0;t<2;t++) {
    for(size_t i=0;i<8;i++) {
      //printf("%2d %04x%04x %04x%04x\n",k,w1,w2,w3,w4);
      tmp=w4;
      w4=w3;
	  w3=w2;
      w2=g(key,k,w1);
      w1=g(key,k,w1)^tmp^(k+1);
      k++;
    }

    for(size_t i=0;i<8;i++) {
      //printf("%2d %04x%04x %04x%04x\n",k,w1,w2,w3,w4);
      tmp=w4;
      w4=w3;
      w3=w1^w2^(k+1);
      w2=g(key,k,w1);
      w1=tmp;
      k++;
    }
  }
  //printf("%2d %04x%04x %04x%04x\n",k,w1,w2,w3,w4);
  
  out[0] = (unsigned char)(w1 >> 8);
  out[1] = (unsigned char)(w1 >> 0);
  out[2] = (unsigned char)(w2 >> 8);
  out[3] = (unsigned char)(w2 >> 0);
  out[4] = (unsigned char)(w3 >> 8);
  out[5] = (unsigned char)(w3 >> 0);
  out[6] = (unsigned char)(w4 >> 8);
  out[7] = (unsigned char)(w4 >> 0);
}

void shift_right(unsigned char v[10])
{
	for (size_t i=0; i<10; ++i)
	{
		if (v[i] == 0) {continue;}

		unsigned int x = (v[i] >> 1);
		unsigned int y = (v[i]  & 1) ? 0x80 : 0;
		
		v[i] = (unsigned char)x;
		if (i < 9)
			v[i+1] = (unsigned char)y;
		
		break;
	}
}

void print_string(const unsigned char s[], size_t len)
{
    for (size_t i=0; i<len; ++i)
        printf("%02x", s[i]);
}

int main(int argc, char* argv[])
{
    unsigned char k[10] = {0x80, 0,0,0,0,0,0,0,0,0};
    unsigned char p[ 8] = {0,0,0,0,0,0,0,0};
    unsigned char c[ 8];
    
    for (unsigned int i=0; i<80; ++i)
    {
        skipjack_encrypt(k, p, c);

        printf("Key: ");
        print_string(k, sizeof(k));
        printf("\n");

        printf("Plaintext: ");
        print_string(p, sizeof(p));
        printf("\n");

        printf("Ciphertext: ");
        print_string(c, sizeof(c));
        printf("\n");

        printf("\n");

        shift_right(k);
    }
    
    return 0;    
}
