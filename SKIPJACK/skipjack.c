...
/*
 * ftp://ftp.funet.fi/pub/crypt/cryptography/symmetric/skipjack/skipjack.c
 */
...
selftest()
{
    ...
    if( memcmp( enc, chk, 8 ) )
		fprintf(stderr, "Skipjack test encryption failed\n");
	//g10_log_fatal("Skipjack test encryption failed\n");
    decrypt_block( &c, dec, enc);
    if( memcmp( dec, inp, 8 ) )
		fprintf(stderr, "Skipjack test decryption failed\n");
...
int main() {
	...
	makeKey(key, tab, 10);
	// encrypt(tab, inp, enc);
	encrypt_block(tab, inp, enc);
	printf((memcmp(enc, chk, 8) == 0) ? "encryption OK!\n" : "encryption failure!\n");
	// decrypt(tab, enc, dec);
	decrypt_block(tab, enc, dec);
