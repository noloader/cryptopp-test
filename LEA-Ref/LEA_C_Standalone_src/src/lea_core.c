...
void lea_set_key_generic(LEA_KEY *key, const unsigned char *mks, unsigned int mk_len)
{
	const unsigned int* _mks = (const unsigned int*)mks;
	...
	{
	case 16:
		key->rk[  0] = ROL(loadU32(_mks[0]) + delta[0][ 0], 1);
		...
		key->rk[  1] = key->rk[  3] = key->rk[  5] = ROL(loadU32(_mks[1]) + delta[0][ 1], 3);
		...
		key->rk[  2] = ROL(loadU32(_mks[2]) + delta[0][ 2], 6);
		...
		key->rk[  4] = ROL(loadU32(_mks[3]) + delta[0][ 3], 11);
	...
	case 24:
		...
		key->rk[  1] = ROL(loadU32(_mks[1]) + delta[0][ 1], 3);
		...
		key->rk[  3] = ROL(loadU32(_mks[3]) + delta[0][ 3], 11);
		...
		key->rk[  4] = ROL(loadU32(_mks[4]) + delta[0][ 4], 13);
		...
		key->rk[  5] = ROL(loadU32(_mks[5]) + delta[0][ 5], 17);
	...
	case 32:
		...
		key->rk[  6] = ROL(loadU32(_mks[6]) + delta[1][ 1],  1);
		...
		key->rk[  7] = ROL(loadU32(_mks[7]) + delta[1][ 2],  3);
